...
public class Facturacion_inicio {
	...
	public void interfaz() {
		...
		txt_cod.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent l) {
				try{
					Producto buscar = new Producto(txt_cod.getSelectedItem().toString());
					Producto producto = Facturacion_inicio.this.controlador.producto_model.hash_productos.search(buscar.generarClaveHash());
			...
			public void actionPerformed(ActionEvent arg0) {
				if(txt_NFact.getText().length()==0 || txt_prod.getText().length()==0 || txt_precio.getText().length()==0 || txt_cant.getText().length()==0)
					...
					try{
						...
						st = db.con.prepareStatement(cad);
		...
		btnRegistro.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					Facturacion_inicio.this.controlador.nuevoCliente();
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	public void actualizarCliente() {
		cmb_clientes.removeAllItems();
		for (int i = 0; i < this.controlador.cliente_model.lista_clientes.size(); i++) {
			cmb_clientes.addItem(this.controlador.cliente_model.lista_clientes.get(i).getNombre());
		}
	}
	
	public void actualizarNumFactura() {
		txt_NFact.setText(Integer.toString(this.controlador.factura_model.factura.getNumFactura()));
	}
}
